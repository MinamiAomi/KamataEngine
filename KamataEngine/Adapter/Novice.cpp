...
class NoviceSystem {
  ...
  private:
	...
	void DrawBox(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, unsigned int color);
	void DrawTriangle(int x1, int y1, int x2, int y2, int x3, int y3, unsigned int color);
	void DrawTriangle3D(const Vector3& v1, const Vector3& v2, const Vector3& v3, unsigned int color1, unsigned int color2, unsigned int color3);
	...
	void DrawSpriteRect(
		int left, int top, int width, int height, int srcX, int srcY, int srcW, int srcH, int textureHandle, float angle, unsigned int color);
	void DrawQuad(
	...
	// 画面クリア時の色
	Vector4 clearColor_ = { 0.1f, 0.25f, 0.5f, 0.0f };
};
...
void NoviceSystem::DrawBox(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, unsigned int color) {
	...
	// 頂点データ
	vertices = {
	  {{(float)x1, (float)y1, 0.0f}, {1, 1, 1, 1}}, // 左上　1--2
	  {{(float)x2, (float)y2, 0.0f}, {1, 1, 1, 1}}, // 右上  | /|
	  {{(float)x3, (float)y3, 0.0f}, {1, 1, 1, 1}}, // 左下  |/ |
	  {{(float)x4, (float)y4, 0.0f}, {1, 1, 1, 1}}, // 右下  3--4
	};
	indices = { 2, 0, 1, 1, 3, 2 };
...
void NoviceSystem::DrawTriangle3D(const Vector3& v1, const Vector3& v2, const Vector3& v3, unsigned int color1, unsigned int color2, unsigned int color3) {
	assert(indexTriangle_ < kMaxTriangleCount);
	...
	// 頂点データ
	vertices = {
	  { v1, FloatColor(color1)},
	  { v2, FloatColor(color2)},
	  { v3, FloatColor(color3)},
	};
...
void NoviceSystem::SetClearColor(unsigned int color) { clearColor_ = FloatColor(color); }
...
void Novice::DrawBox(
	int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, unsigned int color, FillMode fillMode) {
	if (fillMode == kFillModeSolid) {
		sNoviceSystem->DrawBox(x1, y1, x2, y2, x3, y3, x4, y4, color);
	}
	else {
		sNoviceSystem->DrawLine(x1, y1, x2, y2, color);
		sNoviceSystem->DrawLine(x2, y2, x4, y4, color);
		sNoviceSystem->DrawLine(x3, y3, x1, y1, color);
		sNoviceSystem->DrawLine(x4, y4, x3, y3, color);
	}
}
...
void Novice::DrawSpriteRect(
	int left, int top, int width, int height, int srcX, int srcY, int srcW, int srcH, 
	int textureHandle, float angle, unsigned int color) {
	sNoviceSystem->DrawSpriteRect(
		left, top, width, height, srcX, srcY, srcW, srcH, textureHandle, angle, color);
}
...
void Novice::SetClearColor(unsigned int color) { sNoviceSystem->SetClearColor(color); }
...
void Novice::ScreenPrintf(int x, int y, const char* format, va_list& args) {
	sNoviceSystem->ScreenPrintf(x, y, format, args);
}
...
void Novice::DrawTraiangl3D(const class Vector3& v1, const class Vector3& v2, const class Vector3& v3, unsigned int color1, unsigned int color2, unsigned int color3) {
	sNoviceSystem->DrawTriangle3D(v1, v2, v3, color1, color2, color3);
}